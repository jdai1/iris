...
from iris.models import DigestItem, Document, Feedback, Link
...
def populate_digest(session: Session, limit: int = 30) -> list[DigestItem]:
    existing_active = set(
        session.execute(select(DigestItem.document_id).where(DigestItem.status.in_(["queued", "shown"]))).scalars().all()
    )
    dismissed = set(session.execute(select(Feedback.document_id).where(Feedback.action == "dismiss")).scalars().all())
    interest = _interest_vector(session)
    documents = session.execute(
        select(Document).where(Document.document_type == "essay").where(Document.crawl_status == "fetched")
    ).scalars().all()
    ...
    for doc in documents:
        ...
        score = (
            0.48 * cosine(interest, loads_embedding(doc.embedding))
            + 0.32 * (doc.quality_score or 0.0)
        ...
        item = DigestItem(document_id=doc.id, score=score, reason=reason, status="queued")
        session.add(item)
        items.append(item)
...
def get_digest(session: Session, limit: int = 20) -> list[DigestItem]:
    items = session.execute(
        select(DigestItem)
        .where(DigestItem.status == "queued")
...
def record_feedback(
    ...
    *,
    ...
    if digest_item_id:
        ...
        if item:
            item.status = {"save": "saved", "dismiss": "dismissed", "skip": "skipped"}.get(action, item.status)
    session.flush()
    return feedback
