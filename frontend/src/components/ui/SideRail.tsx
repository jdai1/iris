...
export function SideRailSection({ className, ...props }: ComponentProps<'div'>) {
  return <div className={cn('px-2 py-1.5 text-xs font-semibold uppercase text-muted-foreground', className)} {...props} />;
}
...
export function SideRailItem({ active = false, className, ...props }: UiButtonProps & { active?: boolean }) {
  return (
    <Button
      uiVariant="nav"
      className={cn('min-h-8 px-2', active && 'bg-sidebar-accent text-sidebar-accent-foreground', className)}
      {...props}
    />
  );
}
