  ...
  if (!analysis) {
    return (
      <div className="grid gap-4">
        <StateMessage>No profile analysis yet.</StateMessage>
      </div>
    );
  }
...
function ProfileTakeSection({ takes }: { takes: Array<{ take: string }> }) {
  ...
  return (
    <section className="grid gap-2">
      <h4 className="text-sm font-semibold text-foreground">Opinions</h4>
  ...
  return (
    <section className="grid gap-2">
      ...
      <div className="grid gap-1">
        {usable.map((link) => (
          <a key={link.url} href={link.url} target="_blank" rel="noreferrer" className="text-sm font-medium text-primary hover:underline">
