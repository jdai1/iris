...
`questions.jsonl` is the seed eval set. Each row has:

- `id`: stable question id
- `domain`: broad user-intent area
- `query`: the user question to search
- `intent`: what a good answer should help the user do
...
`golden_questions.jsonl` is the corpus-first regression set. These questions were
written after looking at the current indexed blogs, so each row says which
specific article should appear. Use it to catch regressions in retrieval quality
and to tune the LLM judge against human expectations.
...
backend/.venv/bin/python evals/run.py --questions evals/golden_questions.jsonl --out evals/reports/golden.md --json-out evals/reports/golden.jsonl
```
