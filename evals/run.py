...
DEFAULT_QUESTIONS_PATH = ROOT_DIR / "evals" / "datasets" / "questions.jsonl"
...
def load_questions(path: Path) -> list[EvalQuestion]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            missing = {"id", "domain", "query", "tags"} - set(raw)
            if missing:
...
def render_details(results: list[EvalResult]) -> list[str]:
    ...
    for result in results:
        ...
        if result.question.intent:
            lines.extend([f"- Intent: {result.question.intent}", ""])
...
def write_jsonl(path: Path, results: list[EvalResult]) -> None:
    with path.open("w", encoding="utf-8") as handle:
        for result in results:
            ...
            if isinstance(question, dict) and not question.get("intent"):
                question.pop("intent", None)
